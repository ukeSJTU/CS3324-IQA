...
def measure_model_complexity(model_hyper, sample_input):
    ...
    print(f"MSHyperNet FLOPs: {flops_hyper / 1e9:.4f} G")
    print(f"MSHyperNet Params: {params_hyper / 1e6:.4f} M")
...
def main(args):
    ...
    # Load MS-HyperNet model
    model_hyper = models.MSHyperNet(16, 112, 224, 112, 56, 28, 14, 7).to(device)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='MS-HyperIQA: Multi-Scale Image Quality Assessment Demo')
    ...
    parser.add_argument('--model_path', type=str, default='./pretrained/koniq_pretrained.pkl',
                        help='Path to the pre-trained MS-HyperIQA model weights')
