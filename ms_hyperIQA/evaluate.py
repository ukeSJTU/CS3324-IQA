"""
Evaluation script for MS-HyperIQA on test datasets.
...
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate MS-HyperIQA on test datasets')
...
def load_model(model_path, logger):
    """Load MS-HyperIQA model from checkpoint"""
    logger.info(f"Loading MS-HyperIQA model from: {model_path}")
...
def main():
    ...
    logger.info("="*70)
    logger.info("Evaluating MS-HyperIQA (Multi-Scale with FPN and Attention)")
    logger.info("="*70)
